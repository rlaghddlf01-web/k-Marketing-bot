...
def build_easytax_cardnews_scene_prompt(
    ...
    """
    # 🎯 [85mm 망원 인물 렌즈 골든 공식] — 3m 거리 촬영, 8등신 모델 비율, 소두(18% 비율), 가분수/대두 영구 박멸
    golden_85mm_framing = (
        "photographed from 3 meters away with an 85mm professional telephoto portrait lens, "
        "medium cowboy shot, waist-up view showing the complete upper body from head down to hips and belt, "
        "natural 8-head tall realistic adult human body proportions, small head with elegant slender neck and natural broad shoulders, "
    ...
    if slide_idx == 1:
        # 1번: 일상 거실/방, 편안한 캐주얼 사복, 스마트폰 화면 정면 응시
        prompt = (
            f"candid authentic {golden_85mm_framing}of {char}, "
            f"sitting naturally in a bright modern warm sunlit living room with softly blurred bookshelf and warm morning window sunlight in the background with natural depth of field. "
            f"Wearing clean comfortable civilian casual home clothes, a neat cream-colored casual t-shirt. "
            f"The person is holding a sleek modern smartphone vertically in one hand at waist level, "
            f"with the black vertical smartphone screen turned facing directly forward toward the camera. "
            f"Warm genuine confident friendly toothy smile showing clean white teeth, looking directly into the camera lens with authentic trustworthy eye contact. "
            f"Natural unedited skin texture with visible microscopic skin pores, crisp smartphone screen bezel, sharp focus on eyes, authentic documentary photography, 8k"
        )
    elif slide_idx == 2:
        # 2번: 공장/작업장, 실제 작업 유니폼, 스마트폰 절대 금지! 땀 흘리며 성실히 일하는 역동적 3/4 앵글 샷
        prompt = (
            f"authentic candid industrial documentary photo, {golden_85mm_framing}of {continuity} {char}, "
            ...
            f"natural skin texture, subtle authentic forehead sweat glistening, authentic documentary photo, sharp focus, 8k"
        )
    elif slide_idx == 3:
        # 3번: 아늑한 카페/방, 포근한 니트 스웨터, 스마트폰 절대 금지! 좌측 공중 앱 화면을 향해 손짓하며 안도하는 3/4 샷
        prompt = (
            f"authentic candid documentary lifestyle photo, {golden_85mm_framing}of {continuity} {char} positioned on the right half of the frame, "
            ...
            f"same consistent face and hairstyle as slide 1, natural skin texture, warm ambient lighting, sharp focus, 8k"
        )
    elif slide_idx == 4:
        # 4번: 공항 로비/캐리어, 예쁜 여행 사복, 스마트폰 절대 금지! 비행기표/여권 들고 고향 갈 감동의 풀/웨이스트 샷
        prompt = (
            f"authentic candid documentary travel lifestyle photo, {golden_85mm_framing}of {continuity} {char}, "
            ...
            f"same consistent face and hairstyle as slide 1, natural skin texture, warm golden hour ambient lighting, sharp focus, 8k"
        )
    elif slide_idx == 5:
        # 5번: 세련된 카페 테이블, 단정한 셔츠, 스마트폰 절대 금지! 시청자에게 커피잔 앞 엄지척(👍)으로 강력 권유하는 미디엄 샷
        prompt = (
            f"authentic candid portrait lifestyle photo, {golden_85mm_framing}of {continuity} {char} positioned on the right half of the frame, "
            ...
            f"natural skin texture, sharp focus, 8k"
        )
    else:
...
def build_easytax_cardnews_negative_prompt(lang: str, extra: str = "") -> str:
    """
    EasyTax 카드뉴스 전용 부정 프롬프트 (가분수/얼큰이/광각왜곡/밀랍인형/3D CG 원천 차단)
    """
    ...
    distortion_neg = (
        ...
        "3d render, CGI, digital painting, digital illustration, octane render, unreal engine, anime, cartoon, artificial look, over-smoothed skin, "
