...
- 상단 70% (1080 × 945px): 4K 극실사 0원 나눔 현장 사진 배치
- 구분선: 3px 네온 오렌지 라인 (#F97316)
    ...
    def compose_slide(
        ...
        if top_image_path and os.path.exists(top_image_path):
            try:
                ...
                logger.warning(f"K-Market 상단 사진 합성 실패 (솔리드 대체): {e}")
        ...
        # ── A. 상단 배지 & 슬라이드 인디케이터
        badge_text = _sanitize_display_text(card_data.get("badge", f"0원 나눔 꿀팁 {slide_idx:02d}"))
        ...
        draw.text((margin_x + 10, y_cursor), badge_text, font=font_badge, fill=self.TEXT_ORANGE)

        ind_bbox = draw.textbbox((0, 0), indicator_text, font=font_indicator)
        ind_w = ind_bbox[2] - ind_bbox[0]
        draw.text((self.WIDTH - margin_x - ind_w, y_cursor), indicator_text, font=font_indicator, fill=self.TEXT_YELLOW)
        ...
        # ── C. 🎯 [가로 꽉 채움 웜옐로우 서브카피] (36pt ~ 24pt 중앙 정렬)
        ...
        if sub_text:
            y_cursor = self._draw_centered_autofit_text(
                ...
                fill_color=self.TEXT_YELLOW, line_spacing=4, lang=lang
            )
            y_cursor += 8
        ...
        if output_path:
            ...
            logger.info(f"✅ [K-Market 7:3 카드뉴스 렌더링 완료]: {output_path.name}")
