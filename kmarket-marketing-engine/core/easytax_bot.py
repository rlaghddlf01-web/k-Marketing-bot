...
class EasyTaxRefundBot:
    ...
    def run_easytax_cycle(self) -> Dict[str, Any]:
        ...
        # 3. 비자/세금 질문 실시간 스캔 & 안전 유기적 활동 (워밍업 시 정보성 댓글 자동 작성)
        try:
            ...
            logger.info(f"✅ [EasyTax 봇] 레딧 사이클 완료 (팩트안내: {safe_res.get('promo_comments', 0)}, 정보성: {safe_res.get('organic_comments', 0)}, 좋아요: {safe_res.get('upvotes', 0)})")
        except Exception as e:
