...
class CardnewsKMarket:
    ...
    def generate_carousel_cardnews(
        ...
        target_persona_desc = char_anchor if isinstance(char_anchor, str) else char_anchor.get("persona_anchor_desc", "외국인 유학생/근로자")
        ...
        matched_theme = {
            ...
            "name": scenario.get("theme_title", "K-Market 0 Won Giveaway"),
        ...
        landing_url = post_pkg.get("landing_url", f"https://kmarket-service.vercel.app/?lang={lang}")
        ...
        caption_content = (
            f"================================================================================\n"
            f"📢 [K-Market 5장 카드뉴스 — 5대 SNS 알고리즘 맞춤형 원클릭 포스팅 패키지]\n"
            f"• 타깃 언어: {lang.upper()} | 테마: {scenario.get('theme_title', scenario.get('theme_name'))}\n"
            f"• 0원 나눔 공식 링크: {landing_url}\n"
            ...
            f"📝 5장 앨범 전송용 캡션:\n"
            f"{tg.get('caption', '')}\n\n"
            f"🔘 인라인 버튼: [{tg.get('button_text', '0원 나눔 득템하기')}] ➔ {landing_url}\n\n"
        ...
        try:
            ...
            logger.info(f"🚀 [K-Market 카드뉴스 멀티 API 배포 완료] 상태: {list(publish_results.get('platforms', {}).keys())}")
        except Exception as e:
            logger.warning(f"K-Market 카드뉴스 API 배포 경고: {e}")
