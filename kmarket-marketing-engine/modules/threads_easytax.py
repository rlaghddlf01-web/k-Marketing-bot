...
class EasyTaxThreadsPublisher:
    ...
    def _find_cardnews_images(self, lang: str) -> List[str]:
        ...
        if self.desktop_cardnews_dir.exists():
            # 1. 언어 맞춤 하위 폴더 탐색 (예: 이지텍스_vi_*, 이지텍스_VI_*, 이지텍스_uz_*)
            matching_dirs = sorted(
                [d for d in self.desktop_cardnews_dir.glob(f"이지텍스_{lang.upper()}_*") if d.is_dir()] +
                [d for d in self.desktop_cardnews_dir.glob(f"이지텍스_{lang.lower()}_*") if d.is_dir()] +
                [d for d in self.desktop_cardnews_dir.glob(f"*{lang.lower()}*") if d.is_dir()],
            ...
            # 2. 플랫 파일 탐색 (easytax_cardnews_{lang}_s*.jpg) - 자연스러운 1->5 오름차순 정렬
            patterns = [f"easytax_cardnews_{lang.lower()}_s*.jpg", f"easytax_cardnews_{lang.upper()}_s*.jpg"]
            found = []
            for pat in patterns:
                found.extend(list(self.desktop_cardnews_dir.glob(pat)))
            found = sorted(
                list(set(found)),
                key=lambda p: (
                    int(''.join(filter(str.isdigit, p.stem))) if any(c.isdigit() for c in p.stem) else str(p)
        ...
        # 4. 프로젝트 내부 outputs/cardnews/easytax 폴백 탐색
        if len(images) < 5:
