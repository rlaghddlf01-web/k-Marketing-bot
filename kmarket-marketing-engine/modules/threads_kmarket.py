...
from config import BASE_DIR, OUTPUTS_DIR, LANGUAGES, BASE_URLS, DATA_DIR, get_now_kst_str, DESKTOP_CARDNEWS_KMARKET, DESKTOP_THREADS_KMARKET
...
class KMarketThreadsPublisher:
    ...
    def __init__(self, db_mgr: DBManager, supabase_mgr: SupabaseManager):
        ...
        self.desktop_cardnews_dir = DESKTOP_CARDNEWS_KMARKET
        self.desktop_threads_dir = DESKTOP_THREADS_KMARKET
        ...
        for lang in target_langs:
            ...
            if hasattr(self, "desktop_threads_dir") and self.desktop_threads_dir:
                try:
                    ...
                    topic_tag = thread_data.get("topic_tag", "#0원나눔")
                    txt_lines = [
                        "==================================================",
                        f"🛒 [KTRS-Market] 스레드 {time_slot.upper()} 슬롯 ({lang.upper()})",
                        f"🏷️ [스레드 공식 추천 주제 태그]: {topic_tag}",
                        f"타입: {post_type} ({'📸 카드뉴스 5장 첨부' if attached_images else '📝 순수 텍스트 썰'})",
                        f"랜딩 URL: {landing_url}",
                        "==================================================\n"
                    ]
                    if attached_images:
                    ...
                    for idx, p in enumerate(posts):
                        if idx == 0:
                            ...
                            lbl = "2번 팩트/노하우 요약 답글 (1번 글에 답글 달기)"
                        else:
